...
k_br =0.5
 ...
 cQ_up=(CQ_i_up*(K)*K_int_upstream*Nb)+(total_CQ_o_up*Nb)
 CT_up_old = CT_up
...
%v_ind_up_s=sqrt(Ct_up/2)*omga*R*2
v_ind_up_s_0=sqrt(Ct_up/2)*omga*R
...
v_center = (v_ind_up_s/(Y_tip^3))-V_D-v_ind
...
for  i=1:X
    ...
    %Lamda_D_in(i)=((Cl_alpha_down_in(i)*sigma)/16)*((sqrt (1+((32*seta_down_in(i)*r_down_in(i))/(Cl_alpha_down_in(i)*sigma))))-1);
...
cQ_down =((CQ_i_down_in+CQ_i_down_out)*(K)*K_int_downstream)+((total_CQ_o_down_in+total_CQ_o_down_out)*Nb)
                                 
                                    %%% upstream Thrust correction%%%
Lamda_2= (sum (Lamda_down_in))*(delta_r/b)*Nb
%Lamda_2= (sum (Lamda_D))*(delta_r/b)
Lamda_D_tip=Lamda_D(X)
lamda_Tip_relation = (Lamda_D_tip+Lamda_Center)
Ct_downt_relation= (sqrt(Ct_up/2))/(Nb*Y_tip^3)
CT_up_new_co =2*Nb*(Ct_downt_relation-lamda_Tip_relation)^2
if CT_up_new_co > Ct_up
    CT_up_new (k)=  Ct_up/K_int_downstream
else
    CT_up_new (k)=CT_up_new_co/K_int_downstream 
end
...
elseif y == 1 & H < z_venna 
  ...
  for i=1:N
    ...
    %Lamda_D_in(i)=((Cl_alpha_down(i)*sigma)/16)*((sqrt (1+((32*seta_down(i)*r_down(i))/(Cl_alpha_down(i)*sigma))))-1);
 ...
 cQ_down=(CQ_i_down*(K)*K_int_downstream)+(total_CQ_o_down*Nb)
...
v_ind_up_old=sqrt(CT_up_new(k)/2)*omga*R*Nb
